...
startbackup() {
    ...
    if [ "$reply" = y ]; then
        ...
        sudo tar -czpf - --exclude-from=$my_dir/excfile.txt --exclude=$DEST_FOLDER --one-file-system / 2>/dev/null  \
            | pv -s $(du -sb --exclude-from=$my_dir/excfile.txt --exclude=$DEST_FOLDER --one-file-system / 2>/dev/null | awk '{print $1}') > $DEST_FOLDER/$DEST_FILE 
