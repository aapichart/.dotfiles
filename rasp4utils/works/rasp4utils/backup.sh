...
startbackup() {
    ...
    if [ "$reply" = y ]; then
        ...
        sudo tar -czpf - --exclude=$DEST_FOLDER --absolute-names --one-file-system / 2>/dev/null | pv > $DEST_FOLDER/$DEST_FILE 
        ...
        echo "===========> Backup at $(date +'%F %H:%M:%S')......" 
        echo "===========> Backup at $(date +'%F %H:%M:%S')......" > $DEST_FOLDER/backup_log
        sudo tar -cpf - --exclude=$DEST_FOLDER --absolute-names --one-file-system / 2>/dev/null | pv -s $(du -sb --exclude=$DEST_FOLDER / 2>/dev/null | awk '{print $1}' ) > $DEST_FOLDER/$DEST_FILE_UNCOMPRESSED
        # extract content of file to log
        echo "Please be patient, system is creating log file content .... " 
